...
Using a polygon boundary for an area off the coast of Maine where the MCC dynamics of interest occur, we can clip the FVCOM mesh and obtain the indices to use for pulling data from the many netcdf files.
...
```{r}
# Map the area we're using for MCC dynamics:
ggplot() +
...
Then for the ability to see across a broader area, I've prepared a second polygon for clipping the data:


```{r}
#| label: expanded MCC data

# Expanded Area
expanded_mcc_turnoff_coords <- tribble(
  ~"lon", ~"lat",
  -70.65, 42.8,      # Bottom Left
  -70,   43.65,      # Top Left
  -67.25, 44.5,      # Off Jonesport
  -66.45, 43.8) %>%  # Bottom right
  # Close the polygon with the first row
  bind_rows(.[1,])
...
# Trim the mesh to the expanded area
...
```{r}
# Map the expanded area:
ggplot() +
  geom_sf(data = new_england) +
  geom_sf(data = canada) +
  geom_sf(data = expanded_studyarea_mesh, color = "gray60", alpha = 0.4) +
  geom_sf(data = mcc_turnoff_poly, fill = "orange", alpha = 0.4) +
  theme_classic() +
  map_theme() +
  coord_sf(
...
Since I like looking at data all over, I did the same thing for the broader area. This is really not an efficient way to look at the data, but I did it anyways.
...
# For Area weighted PCA, get mesh element areas this way:
...
I am exporting the principal component timeseries from the approach that used the Northward and Eastward current velocities. 

It explained more variance in the data, and mapped fairly well to some intuition about which direction the currents were flowing.
...
The following maps show how the two principal component rotations/loadings correlate with the surface current vectors u (eastward velocity) and v (northward velocity) across along the study area.
...
If we take the principal component values for all locations, and aggregate the loadings across the whole area, we can produce timeseries that reflect the average for the area for each month:
...
# Plot monthly timeseries for the first two prncipal components
# Add a rolling monthly smooth as an overlay
mcc_pc_monthly %>% 
  ggplot() +
  geom_line(
...
The comparison against the three years in the pettigrew paper
...
# DEFUNCT: PCA on Angular Direction and Speed Directly?
