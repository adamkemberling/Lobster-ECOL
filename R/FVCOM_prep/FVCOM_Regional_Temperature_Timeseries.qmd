...
ggplot(aes(time, regional_mu, color = area_id)) +
  ...
  labs(title = "regional Average SST for Lob-ECOL Polygons: 1978 Test")
...
#   #rename(bottom_temp = regional_mu) %>%
#   split(.$area_id) %>%
#   iwalk(
#     function(.x,.y){
...
### Running 2012/2017-2019 

On the first pass I was missing a number of files which were later added. This section covers processing a subset of years and adding them back to the collection.
...
strata_resolutions <- lobster_intersections %>% 
  group_by(strata) %>% 
...
# Join them together:
strata_resolution_data <- left_join(
  mutate(strata_resolutions, strata = as.character(strata)),
  y = strata_points)
...
lobster_strata %>% 
  ...
  left_join(strata_resolution_data) %>% 
  ggplot() +
...
Timeseries can now be evaluated:
