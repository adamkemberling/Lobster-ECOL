...
This document will mirror the regional surface temperature processing doc, but will use monthly mean data from thredds. For each of these data endpoints we will do the following things:
...
average_from_nodes <- function(
  ...
  triangular_element_means <- pmap(fvcom_mesh_trios, function(elem, p1, p2, p3) {
    p1_ts = var_slice[p1]
    p2_ts = var_slice[p2]
    p3_ts = var_slice[p3]
...
all_region_salinity <- imap_dfr(
  ...
  .f = function(nc_x, nc_date){
    ...
    # 3.  Process the Average Within Each Region
    # Surface Salinity
    ...
    # Join Surface and Bottom into one table
