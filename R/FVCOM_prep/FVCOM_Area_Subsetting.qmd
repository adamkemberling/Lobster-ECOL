...
plot(sf::st_geometry(mesh), border = scales::alpha("gray", 0.6))
...
p2 <- ggplot(gom_vars_t) +
  ...
  scale_fill_distiller(palette = "RdBu", limits = c(0,9)) +
  map_theme() +
  labs(title = str_c("Surf Temp Avg. from ", time_dim[1], " to ", time_dim[length(time_dim)]))
...
p3 <- ggplot(gom_vars_bot) +
  ...
  map_theme() +
  labs(title = str_c("Bot Temp Avg. from ", time_dim[1], " to ", time_dim[length(time_dim)]))
p3
```
...
ggplot(gom_vars) +
  ...
  geom_vector(
    ...
    pivot = 0, preserve.dir = TRUE, direction = "ccw", color = "gray20", alpha = 0.25)+
  scale_fill_distiller(palette = "RdBu") +
  map_theme()
...
gom_elems_regular <- raster::stack(
  sapply(
    ...
    elem_var_list, 
    function(f) {
      fvcom::rasterize(gom_vars, field = f) 
        }, 
    simplify = FALSE))
...
# What if we base it off nodes instead of elements
get_node_mesh_geometry(x, what = "lonlat", where = "nodes")
...
# st_drop_geometry(mesh) == st_drop_geometry(gom3_mesh)
```
...
gom_mesh   <- mesh_trim(mesh = mesh, domain = gom_poly)
gb_mesh    <- mesh_trim(mesh = mesh, domain = gb_poly)
#shelf_mesh <- mesh_trim(mesh = mesh, domain = shelf_poly)

# Check them
ggplot() +
  geom_sf(data = vts_mesh, aes(color = "mesh"))+
...
# Testing Multiple Dates with Target Mesh

So where are we at:
 * Need to write some code that can open a connection for an area we care about \
grab 1-4 variables at the surface, and the ones at the bottom \
 * average for the month \
 * and spit them out in a table \
...
Depth should be a dimension that we can pass into ncvar_get to prevent getting data we'll later discard. Need to sort out the "start" and "count" functionalities.

```{r}
# Do some digging to see how we should approach the depth situation 
# And whether we can throw integers in for depth...
# It Doesn't report units because they change

# What depth layers do we want
depths <- ncvar_get(gom3_x, "siglev")

# There are 46? depth (siglev), and they exist at all the centroids, 
# but their spacing is not consistent (stretches for deep areas, compresses for shallow)
# Their units are also meaningless
dim(depths)
range(depths)

# First one is surface, 45/46thth might be bottom
depths[,1]
depths[,46]
```
...
```{r}
# Manual lookup of start/stop indices, weird behavior from time zones
x_times<- fvcom_time(gom3_x)
which(x_times == "1978-01-01 00:00:00 UTC")



# Getting Data for specific mesh:
# These are the core variables at the nodes
node_var_list <- c('temp', 'salinity')


# Then Can we Grab what we want?
# yea kinda. except temp is something with values at each node, and this reports it as if its the space between
mesh_vars <- get_mesh(
  x, # Dataset lazyloaded with ncdf4 from THREDDS 
  y = 1, # integer, or indices for siglay or siglev (depth indices)
  vars = "temp",  # Variables we want
...
```{r}
# So matt gets variables this way:
# Get variable data
var.fvcom<- ncvar_get(nc, varid = variable, start =  c(1, 45, 1))

# And I'm really thrown off by the 45

#-----------------

# Getting variables using ncdf4 approach
gom3_x$dim
#  Documentatiu=on says the dimensions go XYZT
ncvar_get(gom3_x, varid = "temp", start =  c(1, 45, 1, 1))
