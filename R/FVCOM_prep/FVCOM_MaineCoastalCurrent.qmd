...
gom3_early$var$u$longname
gom3_early$var$v$longname
...
Each lat/lon pair can be given a unique ID. We can then pivot wider so that ID-variable matches each get one row per month. Then we can do the PCA
