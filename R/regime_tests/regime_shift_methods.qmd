...
# Test timeseries
tester <- filter(lobecol_fvcom, area_id == "GOM_GBK")
...
Since the initial paper, Rodionov added a number of anhancements to the approach which improve its ability to detect true shifts in the mean in the presence of outliers, autocorrelation (red-noise), and trends.
...
One pre-processing step done by Rodionov in newer STARS versions (the excel macro version) is an accountance of variability related to "red noise" processes, which can appear as regimes themselves and interfere with regime shift identification.
...
What about prewhitening? Prewhitening describes aproaches used to remove "red-noise" from a time series before analysis. The methods evaluated in Stirnimann et al. 2019 that were added in the Rodionov 2006 study include:
...
# Preprocessing Topics: Seasonality / Periodicity and Trends

From Lund et al. 2023

>To account for correlation in an AMOC changepoint analysis, prewhitening first fits a pth-order autoregressive [AR(p)] model to the series (this assumes nonperi-
odic data).
...
tester %>% 
  ...
  labs(title = "Detrending + Prewhitening of Bottom Temperature")

```

We can then re-run STARS and see if the shifts we were seeing before have changed. They have not really, which could point to a variance shift issue.

```{r}
# Regime Shift Test on prewhitened timeseries
detrend_regimes <- rshift::Rodionov(
  data = tester,
  col = "detrend_prewhitened", 
  time = "time", 
  l = 365*5, 
  prob = 0.05, 
  startrow = 1, 
...
# # Plot 
# ggplot(t1_regimes) +
#   geom_line(aes(time, RSI))

# Plot the RSI
RSI_graph(
  data = detrend_regimes, 
  col =  "detrend_prewhitened", 
  time =  "time", 
  rsi =  "RSI", 
  mean_lines = TRUE)
...
bot_temp_shifts %>% 
  ggplot() +
  #geom_line(aes(time, bottom_t_pw, color = area_id), alpha = 0.1, linewidth = 0.2) +
  geom_line(aes(time, regime_mu_pw, color = area_id), linewidth = 0.35) +
  geom_hline(yintercept = 0, color = "gray") +
  facet_grid() +
  labs(
    x = "Time",
    y = "Prewhitened Regime Means",
    title = "")
```
...
test_envcpt <- envcpt(tester$bottom_t, minseglen = 365*3)
...
Here is that same timeseries after detrending:
