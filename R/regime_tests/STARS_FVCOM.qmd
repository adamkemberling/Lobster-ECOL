...
trend_tester %>%
  ...
  geom_line(
    aes(time, bottom_t),
    color = "gray60", alpha = 0.45) +
  geom_line(
    aes(time, trend_resid + coef(tester_trend_mod)[[1]]), 
    color = "gray20", alpha = 0.65) +
  geomtextpath::geom_labelsmooth(
    aes(time, bottom_t, color = "Bottom Temperature", label = "Original"),
    method = "lm", se = F,
    color = "gray50") +
  geomtextpath::geom_labelsmooth(
    method = "lm", se = F,
    aes(time, trend_resid + coef(tester_trend_mod)[[1]], label = "Trend-Removed"),
    color = "gray10") +
