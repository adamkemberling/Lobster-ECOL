    ...
    )
{
  ...
  for (TIMESERIESindex in 2:length(TS[1, ])){
    ...
    #} # no more filtereddata option
    
    
    # Prewhitened timeseries regime averages
    # Uses matrix X
    # suffix "_pw" for results from these values
    vRMean_pw <- 0 # set constant / initial value
    if (preWhitening == T){
    ...
    if(preWhitening == T){
      ...
      # Take dimenions of non-prewhitened mean matrix
  ...
  # Take time/date from data.timeseries
  ...
  if(returnResults == T){
    ...
    if(preWhitening){
      ...
      RMean_mat_pw_save <- as.data.frame(RMean_mat_pw)
      RMean_mat_pw_save[,1] <- data.timeseries[,1]
      # timeseries prewhitened
      # prewhitened means
      dat_all <- dat_all %>%  
        left_join(TabTSpw_save) %>%    
        left_join(RMean_mat_pw_save)
      }
